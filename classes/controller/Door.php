 ...
 public function action_doorInfo($id_door = false)
{
    ...
    $data = array(
        ...
        // 'events' => Model::Factory('Door')->event_door($id_door), // <-- УБИРАЕМ ЭТУ СТРОКУ
    ...
    $this->template->content = $content;
		...
		public function action_getEvents()
		{
			...
			$events = Model::Factory('Door')->event_door($id_door, $timeFrom, $timeTo);
			...
			foreach ($events as $event) {
				$data[] = array(
					...
					'PEOPLE_URL' => URL::site('people/peopleInfo/' . Arr::get($event, 'ID_PEP') . '/' . Arr::get($event, 'ID_CARD'))
				);
			}
